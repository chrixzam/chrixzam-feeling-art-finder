...
async function searchAIC(q, limit = MAX_RESULTS) {
  // AIC API caps limit to ~100
  const capped = Math.min(limit, AIC_MAX_LIMIT)
  try {
    ...
    searchUrl.searchParams.set('limit', String(capped))
...
export default function App() {
  ...
  async function handleSearch(e) {
    ...
    setLoading(true)
    try {
      // Make the two searches resilient to individual failures
      const initial = await Promise.allSettled([searchMet(q), searchAIC(q)])
      let metItems = initial[0].status === 'fulfilled' ? initial[0].value : []
      let aicItems = initial[1].status === 'fulfilled' ? initial[1].value : []
      ...
      if (metItems.length === 0 && aicItems.length === 0 && terms.includes('painting')) {
        ...
        if (fallbackQ) {
          const fb = await Promise.allSettled([searchMet(fallbackQ), searchAIC(fallbackQ)])
          metItems = fb[0].status === 'fulfilled' ? fb[0].value : []
          aicItems = fb[1].status === 'fulfilled' ? fb[1].value : []
      ...
      if (items.length < 50) {
        for (const term of terms) {
          const more = await Promise.allSettled([searchMet(term), searchAIC(term)])
          const m = more[0].status === 'fulfilled' ? more[0].value : []
          const a = more[1].status === 'fulfilled' ? more[1].value : []
          for (const it of [...m, ...a]) {
            if (!seen.has(it.id)) {
              seen.add(it.id)
              items.push(it)
